    ...
    if( g_TwMgr->m_CurrentXDisplay!=NULL && g_TwMgr->m_CurrentXWindow!=None )
    {
        ...
        if( XGetSelectionOwner(Dpy, ClipboardAtom)!=None )
        {
            ...
            while( g_TwMgr->m_Timer.GetTime()-StartTime < 0.5 )
            {
                ...
                // If we're our own CLIPBOARD owner (e.g. we copied from this
                // same process earlier), the SelectionRequest generated by
                // our own XConvertSelection() above is delivered to this
                // same window/connection - nothing else services it while
                // we're blocked here, so without this we'd always spin for
                // the full timeout below and never see a reply. Answer it
                // ourselves so the request completes immediately instead.
                XEvent SelfRequest;
                while( XCheckTypedWindowEvent(Dpy, Win, SelectionRequest, &SelfRequest) )
                    TwHandleX11SelectionRequest(&SelfRequest);
                usleep(1000);
            }
